...
def _to_4d_tensor(image, data_range):
    ...
    if tensor.ndim == 2:
        ...
        raise ValueError(f"Input images should be 2-d, 3-d, or 4-d, but got {tensor.shape}")

    if tensor.max() <= 1.0 and data_range > 1:
        tensor = tensor * data_range
    return tensor
...
def ssim(X,
    ...
    # 需要扩展为 [B, C, H, W]
    X = _to_4d_tensor(X, data_range)
    Y = _to_4d_tensor(Y, data_range)
